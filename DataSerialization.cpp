...
int DataSerialization::serializeToFileList(std::ostream* output, const std::vector<File> &files) {
  shorrent::FileList fileList;
  for (auto &file: files) {
    shorrent::File *file_p = fileList.add_files();
    DataSerialization::serializeToFile(file, *file_p);
  ...
  std::string str_out;
  fileList.SerializeToOstream(output);
  return 0;
}
...
int DataSerialization::deserializeFileList(std::istream *input, std::unique_ptr<std::vector<File>> &files_p) {
  shorrent::FileList fileList;
  fileList.ParseFromIstream(input);
  files_p.reset(new std::vector<File>());
  for (int i = 0; i < fileList.files_size(); i++) {
    std::unique_ptr<File> file_p;
    DataSerialization::deserializeFile(fileList.files(i), file_p);
